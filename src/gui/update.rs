    ...
    match message {
        ...
        Message::Interaction(Interaction::CatalogInstall(source, flavor, id)) => {
            ...
            if let Some(addon_path) = ajour.config.get_addon_directory_for_flavor(&flavor) {
                return Ok(Command::perform(
                    perform_fetch_latest_stable_addon(source, id, empty_addon, addon_path, flavor),
...
/// Unzips `Addon` at given `from_directory` and moves it `to_directory`.
async fn perform_fetch_latest_stable_addon(
    ...
    let result = match source {
        catalog::Source::Curse => {
            curse_api::latest_stable_addon_from_id(source_id, addon, addon_path, flavor).await
        }
        catalog::Source::Tukui => {
            tukui_api::latest_stable_addon_from_id(source_id, addon, addon_path, flavor).await
        }
    };
